  ...
  <v-container class="mb-12 pb-12" v-if="companyLists.length>0">
    ...
    <v-card class="mt-5">
      ...
      <v-row class="ma-0" style="margin-top:-30px;">
        ...
        <v-col :cols="liga=='https://backendmf.unocrm.mx/'?'3':'4'">
          <v-autocomplete
            outlined dense @keydown.enter="filter()"
        ...
        <v-col :cols="liga=='https://backendmf.unocrm.mx/'?'3':'0'">
          <v-text-field outlined dense v-if="liga=='https://backendmf.unocrm.mx/'" v-model="quotation.purchase_order" label="Orden de Compra"/>
        </v-col>

        <v-col :cols="liga=='https://backendmf.unocrm.mx/'?'3':'4'">
          <v-menu v-model="datePicker" :close-on-content-click="false" :nudge-right="40" transition="scale-transition" offset-y min-width="290px">
            <template v-slot:activator="{ on }">
              <v-text-field :rules="[v => !!v || 'Campo requerido']" clearable required
                v-model="quotation.date" label="Fecha Programada" outlined dense
      ...
      <v-row class="px-2 ma-0 py-2" v-for="(quotation_item,k) in quotation.items" :key="k">
        <v-col ols="12" sm="4" md="2" class="py-0 my-0">
          <v-text-field :suffix="quotation_item.item!=''?quotation_item.unit:''" type="number" v-model="quotation_item.quantity" label="Cantidad"/>
        </v-col>
      ...
      <div class="px-6 pb-8">
        <v-btn class="elevation-0" block :loading="gris" :disabled="gris" @click="save()" :dark="!gris" color="#5d267b">
          Realizar Pedido
        </v-btn>
      </div>
    </v-card>
...
export default {
  ...
  methods:{
    roundPesos(n){ return Math.round(n); },
    ...
    repriceItem(it){
      ...
      it.price       = it.price = this.roundPesos(base).toFixed(2);
      it.total_price = (price * qty).toFixed(2)
    },
    repriceAllItems(){
  ...
  watch:{
    ...
    // Cambios en items (producto o cantidad)
    productTotal:{
      handler(){
        if(this.liga === 'https://backendmf.unocrm.mx/'){
          this.repriceAllItems()
